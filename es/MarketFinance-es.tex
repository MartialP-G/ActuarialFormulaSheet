...
Se enumeran \textbf{valores} como acciones o bonos, \textbf{fondos} ((\engl{Exhange-Traded Funds} que replican índices de acciones, ETC o ETN que replican índices o materias primas más específicos, SICAV o FCP, bonos de suscripción, \engl{warrant}), \textbf{contratos de futuros}, \textbf{opciones}, \textbf{swaps} y \textbf{productos estructurados}.
...
Para un tipo de interés \(i\), la duración de Macaulay se expresa así:
\[ 		
D = \sum_{t=1}^{n} t \cdot w^i_t, \quad \text{où} \quad w^i_t = \frac{PV^i(C_t)}{P(i)}.
 \]	
...
Este modelo se extiende a \(n\) periodos con \(n+1\) precios posibles para \(S_T\).  Cuando \(N=1\), al vencimiento, el valor de una opción de compra viene dado por \( C_{1}^{u} = (S_{1}^{u} - K)^+ \) y \( C_{1}^{d} = (S_{1}^{d} - K)^+ \).
...
\[q = \frac{(1 + i_{f}) - d}{u - d}>0\]

\textbf{Precio de Call} (con \( S_{1}^{d} < K < S_{1}^{u} \), por lo que \(C_d=0\)) :
\[
...
Ellas se ilustran en la figura donde los pictogramas \shorthandoff{"}\begin{tikzpicture}[xscale=0.2, yscale=-0.2]
...
\end{tikzpicture}\shorthandon{"} representan los diferentes vencimientos habituales para este tipo de propiedad o inversión.
...
\[
\text{SW}_{\text{pagador}} = A(T) \left[ S_0\Phi(d_1) - K \Phi(d_2) \right]
\]
or :
\[
\begin{aligned}
...
\end{aligned}
\]
y \(\Phi(\cdot)\) es la función de distribución acumulativa de la distribución normal estándar.

\textbf{Fórmula para un swaption de receptor} :
\[
\text{SW}_{\text{receptor}} = A(T) \left[ K \Phi(-d_2) - S_0 \Phi(-d_1) \right]
\]
