...
Notez que certaines références donnent le paramètre de forme, comme \(\kappa = - \xi\).
...
\begin{itemize}
	...
	\end{itemize}
\end{itemize}
Après ces corrections, les données sont dites as if (en économie, on utilise l'expression contre-factuel).
...
\begin{f}[La prime \emph{Burning Cost}]
	...
	\(X_{i}^{j}\) désigne le \(i^{e}\) sinistre  de l'année \(j\) as if actualisé, revalorisé et  redressé,
