...
\begin{f}[Variables aleatorias]
	...
	Sea \((\Omega, \mathcal{A}, \mathbb{P})\) un espacio de probabilidad. Llamamos \textbf{variable aleatoria} \(X\) desde \(\Omega\) hasta \( \Re\) cualquier función medible \(X:\Omega\mapsto \Re\).
	...
	\]
	donde el lado derecho representa la probabilidad de que la variable aleatoria real \(X\) tome un valor menor o igual a \(x\).
	La probabilidad de que \(X\) esté en el intervalo \([a, b]\) es por tanto, si \(a< b\),
	\(
	\mathbb{P}(a< X\leq b)\ =\ F_X(b)-F_X(a)
	\)
	
	Una ley de probabilidad tiene una \textbf{densidad de probabilidad} \(f\), si \(f\) es una función definida en \(\mathbb{R}^{+}\), integral de Lebesgue, tal que la probabilidad del intervalo \([a, b]\) está dada por
	\[
	\mathbb{P}(a< X\leq b)=\int_a^b f(x) \mathrm{dx} \mbox{ para todos los números tq }a<x<b.
	\]
\end{f}
\hrule
\begin{f}[Esperanza]{\ }
...
\begin{f}[Théorèmes fondamentaux] {\ }
	...
	En reprenant les notations, la \textbf{loi forte des grands nombres} précise que \((Y_n)_{n\in\mathbb{N}}\) converge vers \(E(X)\) presque sûrement.
	%
	\[
