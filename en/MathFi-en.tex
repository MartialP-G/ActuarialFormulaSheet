...
is the annual reference rate \(i^{(m)}\) (or \textbf{face} rate), which can be converted into a periodic rate over \(m\in \mathbb{N} \geq 1\) periods.
\smallskip

\emph{Periodic rate \(i_m=i^{(m)}/m\)}

corresponds to the nominal rate divided by the number of periods \(m\) in a year.
If the periodic rate is weekly, it will be equal to the nominal rate divided by 52.
\smallskip

\emph{The \textbf{annual} effective rate \(i_a\)}

is obtained by compounding the periodic rate over one year
\[
i_a=\left( 1+i_m\right)^{m}-1=\left( 1+\frac{i^{(m)}}{m}\right)^{m}-1
\]

\emph{Effective rate \textbf{per period} \(i_e\)}

Conversely, when the \textbf{effective annual} rate \(i_a\) is known, the effective rate corresponding to a single period is 
\[
i_e=\left( (1+i_a)^{1 / m}-1\right) 
\]

\(i\) may be negative, but we shall assume \(i>-1\), so that the accumulation factor \(1+i\) remains strictly positive.
\end{f}
\hrule
...
\end{tikzpicture}
\end{center}
The annuity \(K_k=K,\) \(\forall k\) for a capital amount \(S_0\), based on a rate \(i\), is then equal to:
\[
