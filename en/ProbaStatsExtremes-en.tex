...
\]
%à nouveau, pour  \(x \geq \mu\), et \(x \leq \mu - \sigma /\xi\) where \(\xi < 0\). 
...
If the failure rate decreases over time, then \(\alpha<1\). If the failure rate is constant over time, then \(\alpha=1\). If the failure rate increases over time, then \(\alpha>1\). Understanding the failures rate can provide insight into the cause of failures.
...
The distribution function of \textbf{Frechet's law}\index{D\'efinition! loi de Frechet} is given by
\[
...
\begin{f}[Sub-exponential density]
	...
	\[
	T_n(p)=\frac{X_{(1:n)}+X_{(2:n)}+\ldots + X_{(np:n)}}{\sum_{1\leq i\leq n}(X_i)} \mbox{ where } \frac{1}{n}\leq p\leq 1
	\]
