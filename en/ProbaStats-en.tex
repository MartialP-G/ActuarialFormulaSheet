...
\begin{f}[Axiomatic]{\ }
	...
	\begin{description}
		\item[(P1)] \(A \in  \mathcal{A} \)    then  \( \mathbb{P}(A) \geq 0\),
		\item[(P2)] \( \mathbb{P}(\Omega) = 1\),
		\item[(P3)] \(A, B \in  \mathcal{A}\),  if  \(A\cap B =\varnothing\)    then   \(\mathbb{P}(A\cup B)=\mathbb{P}(A) + \mathbb{P}(B)\).
	\end{description}
...
\begin{f}[Random variables]
	...
	A \textbf{real random variable} is any measurable function 
	...
	\]
	More generally, a \textbf{random variable} taking values in 
	a measurable space \((E, \mathcal{E})\) is any measurable mapping 
	\(X: (\Omega, \mathcal{A}) \to (E, \mathcal{E})\), that is, such that:
	\[
	...
	It fully characterizes the distribution of \(X\). It is increasing, continuous from the right, and satisfies
	\[
	...
	The variable \(X\) has a  \textbf{probability density} \(f\) if there exists a Lebesgue-integrable function \(f: \mathbb{R} \to \mathbb{R}_+\) such that
	\[
	F_X(x)=\int_{-\infty}^x f(t)\,dt,
	\]
	and therefore, for all \(a<b\),    
	\[
...
\begin{f}[Expectations]{\ }
	...
	We assume throughout the rest of this section that the expectation of the random variable \(X\) exists.
	...
	\]
	where \(\mathbb{P}(X=x_j)\) is the probability associated with the value \(x_j\).
	
	The mathematical expectation in the continuous case~:
	\[
	\mathbb{E}[X]=\int_{-\infty}^{\infty} x\, f(x)\,dx
	\]
	where \(f\) exists and denotes the probability density function of the random variable \(X\), defined here on \(\mathbb{R}\).
	
	Since it involves a sum or an integral, the expectation is linear, that is:
	\[
	\mathbb{E}\left[c_0+c_1X_1+c_2X_2\right]=c_0+c_1\mathbb{E}[X_1]+c_2\mathbb{E}[X_2]
	\]
	
	Finally, when the objects exist,
	\[
...
\]
where \(F^+(\boldsymbol{x}) = \min \{F_1(x_1),F_2(x_2)\}\), et 
...
%	\[
%	S_t = S_0 \exp(X_t), \quad \text{where } X_t = (\mu - \frac{\sigma^2}{2})t + \sigma W_t
%	\]
%	
...
\begin{tikzpicture}[domain=-4:10,scale=0.6]
	...
	\draw[name path=g,color=BleuProfondIRA,smooth] plot  ({\x},{60*normal(\x,3,3)})   node[above=3.5,left] {\(K\times g(x)\), normal distrib. density };
...
\begin{f}[Cross-validation]
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Evaluate the performance (error, log-likelihood...) on the \(k^{\text{th}}\) block.
		\end{itemize}
