...
For a rate \(i\), the Macaulay duration is given by:
\[ 		
D = \sum_{t=1}^{n} t \cdot w^i_t, \quad \text{where} \quad w^i_t = \frac{PV^i(C_t)}{P(i)}.
 \]	
...
This model extends to \(n\) periods with \(n+1\) possible prices for \(S_T\). With \(N=1\), at expiration, the value of a call option is given by \( C_{1}^{u} = (S_{1}^{u} - K)^+ \) and \( C_{1}^{d} = (S_{1}^{d} - K)^+ \).
...
\textbf{Call price} (with \( S_{1}^{d} < K < S_{1}^{u} \), then \(C_d=0\)):
\[
