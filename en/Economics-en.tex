...
\begin{f}[Utility function]
	...
	\begin{itemize}
		...
		\item \textit{CRRA}  (const. relative risk aversion): \(u(x) = \frac{x^{1 - \gamma}}{1 - \gamma}\), \(\gamma \ne 1\)
...
\begin{f}[Risk aversion]
	An agent is said to be strictly \textbf{risk-averse} (or risk-phobic) if:
	\[
...
\begin{f}[Diversification and utility]
	...
	\[
	\max \mathbb{E}[u(X)], \quad \text{where } X = \sum_{i} w_i X_i, \quad \text{s.c. } \sum w_i = 1
	\]
...
\textbf{Insurance with deductible} \(d\) (self-insurance portion):
The insurer pays an indemnity \(I(l)=(l-d)_{+}\) knowing \(L=l\).
\[
...
\textbf{Moral hazard:}
Insuring the risk reduces or interrupts the insured's efforts. The efforts of
\begin{itemize}
