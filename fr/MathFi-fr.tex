...
\emph{Intérêt simple \(i\) (à partir d'une valeur \(V_0\))}
\[
I_t=V_0 i t=V_0 i \frac{k}{365}\ \ t \text{ en année et } k\text{ en jours.}
\]


\emph{Intérêt composé \(i\)}
\[
V_n=V_0(1+i)^n%=P\left(1+\frac{p}{100}\right)^n
\]
...
est le taux annuel de référence \(i^{(m)}\) (ou taux \textbf{facial}), convertible en taux périodique sur \(m\in \mathbb{N} \geq 1\) périodes.
\smallskip

\emph{Taux périodique \(i_m=i^{(m)}/m\)}

correspond au taux nominal divisé par le nombre de périodes \(m\) sur un an.
Si le taux périodique est hebdomadaire, il sera égal au taux nominal divisé par 52.
\smallskip

\emph{Le taux effectif \textbf{annuel} \(i_a\)}

 résulte de la composition du taux périodique sur un an
\[
i_a=\left( 1+i_m\right)^{m}-1=\left( 1+\frac{i^{(m)}}{m}\right)^{m}-1
\]

\emph{Taux effectif \textbf{par période} \(i_e\)}

Réciproquement, lorsque le taux \textbf{effectif annuel} \(i_a\) est connu, le taux effectif correspondant à une période est 
\[
i_e=\left( (1+i_a)^{1 / m}-1\right) 
\]

\(i\) peut être négatif, mais on supposera \(i>-1\), de sorte que le facteur d'accumulation \(1+i\) reste strictement positif.
\end{f}
\hrule
...
La principale propriété de l'emprunt est de considérer séparément les intérêts du remboursement (ou de l'amortissement).
...
\end{tikzpicture}
\end{center}
L'annuité \(K_k=K,\) \(\forall k\) pour un capital \(S_0\), sur la base d'un taux \(i\), est alors égale à :
\[
