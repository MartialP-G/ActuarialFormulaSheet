...
Voici les \(n=8\) polices sinistrées (\(1\leq i \leq n\)), les sinistres des autres polices  étant  nuls (\(S_i=0, \forall i>n\)) :
...
L'assureur fixe la priorité \(a\) et la portée \(b\) (respectivement 2M\EUR{} et  4M\EUR{} dans la figure).
\[
...
\begin{center}
	...
	\begin{tikzpicture}
		...
		\begin{axis}[axis x line=bottom, axis y line = left,ymin=0,ymax=24,xmin=0, xmax=10, height=7cm,width=5cm, xticklabel style ={font=\footnotesize,align=center},  yticklabel style ={font=\footnotesize}, 
			...
			\legend{Sin Conservés,Sin cédés,, Priorité 2M€, Portée 4M€ }
		\end{axis}	
	\end{tikzpicture}
	%	
	\begin{tikzpicture}
...
Il s'exprime sur la base du ratio \(S/P\) avec une priorité et une portée du \(XL\) exprimées en \%.
...
La clause de superposition a pour effet de recalculer les bornes du traité, car un même évènement peut engager simultanément les traités des exercices de souscription \(N\) et \(N-1\).
\end{f}
\hrule
...
Le déclencheur peut être lié directement aux résultats de la cédente (Indemnitaire), dépendre d'un indice de sinistralité, d'un paramètre mesurable (somme des excédents de pluie, échelle de Richter, taux de mortalité), ou d'un modèle (RMS \& Equecat Storm modelling). 
