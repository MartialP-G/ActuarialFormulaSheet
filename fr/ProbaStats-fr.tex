...
\begin{f}[Axiomatique]{\ }
	...
	satisfaisant aux propriétés (ou axiomes) suivantes~:
	\begin{description}
		\item[(P1)] \(A \in \mathcal{A} \)    alors  \( \mathbb{P}(A) \geq 0\),
		\item[(P2)] \( \mathbb{P}(\Omega) = 1\),
		\item[(P3)] \(A, B \in \mathcal{A}\),  si  \(A\cap B =\varnothing\)    alors   \(\mathbb{P}(A\cup B)=\mathbb{P}(A) + \mathbb{P}(B)\).
	\end{description}
...
On appelle \textbf{variable aléatoire réelle} toute application mesurable 
...
\]
Plus généralement, on appelle \textbf{variable aléatoire} à valeurs dans 
un espace mesurable \((E, \mathcal{E})\) toute application mesurable 
\(X : (\Omega, \mathcal{A}) \to (E, \mathcal{E})\), c'est-à-dire telle que :
\[
...
\]
	...
	Elle caractérise entièrement la loi de \(X\). Elle est croissante, continue à droite, et vérifie
	\[
...
La variable \(X\) admet une  \textbf{densité de probabilité} \(f\) s’il existe une fonction \(f : \mathbb{R} \to \mathbb{R}_+\), Lebesgue-intégrable, telle que
\[
F_X(x)=\int_{-\infty}^x f(t)\,dt,
\]
et donc, pour tout \(a<b\),	
	\[
	\mathbb{P}(a< X\leq b)=\int_a^b f(x) \mathrm{dx} \mbox{ \  }\forall\ a<b.
	\]
...
On suppose dans toute la suite que l'espérance de la variable aléatoire \(X\) existe.
...
\]
où \(\mathbb{P}(X=x_j)\) est la probabilité associée à la valeur \(x_j\).

L'espérance mathématique dans le cas continu~:
\[
\mathbb{E}[X]=\int_{-\infty}^{\infty} x\, f(x)\,dx
\]
où \(f\) existe et désigne la fonction de densité de la variable aléatoire \(X\), définie ici sur \(\mathbb{R}\).

S'agissant d'une somme ou d'une intégrale, l'espérance est linéaire, c'est-à-dire~:
\[
\mathbb{E}\left[c_0+c_1X_1+c_2X_2\right]=c_0+c_1\mathbb{E}[X_1]+c_2\mathbb{E}[X_2]
\]

Enfin, lorsque les objets existent,
\[
...
\begin{f}
	...
	Soit \(N\) une variable aléatoire discrète dans \(\mathbb{N}^+\), \(\left(X_i\right)\) une suite de variables aléatoires \(iid\) d'espérance et variance finies, alors pour  \(\displaystyle S=\sum_{i=1}^{N}X_i\) :
	\[
...
Dans la méthode du \textbf{changement de variable}, on suppose qu'on sait simuler une loi \(X\), et qu'il existe \(\phi\) tel que \(Y=\varphi(X)\) suit une loi \(F_Y\). L'exemple naturel est celui de \(X\sim \mathcal{N}(0,1)\) et de faire le changement \(Y=\exp(X)\) pour obtenir Y qui suit une loi lognormale.
...
\begin{f}[Validation croisée]
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Évaluer la performance (erreur, log-vraisemblance...) sur le \(k^e\) bloc.
		\end{itemize}
